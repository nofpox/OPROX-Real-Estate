...
import type { CustomField, CreateCustomFieldInput, UpdateCustomFieldInput } from "@/lib/local-hooks";
...
function AppearanceTab() {
  ...
  async function handleLogoUpload(file: File) {
    ...
    setUploading(true);
    try {
      // Local standalone: use base64 data URL
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <div>
              <p className="font-semibold">Esteti In Preview Links</p>
              <p className="text-xs text-muted-foreground mt-0.5">
                Temporary 1-hour access links — Esteti In only
              </p>
            </div>
            <Button
              size="sm"
          ...
          {showForm && (
            <div className="bg-muted/60 rounded-xl p-4 space-y-3 border border-border/60">
              ...
              <Button className="w-full" onClick={generate} disabled={genLoading}>
                {genLoading ? "Generating…" : "Create Esteti In Preview Link"}
              </Button>
            </div>
          )}
