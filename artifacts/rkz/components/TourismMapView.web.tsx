 ...
 * Filter buttons inside the iframe fetch /api/poi on click.
 * type=apartment pulls from apartments table; popup has Uber/Bolt/Careem buttons.
 */
...
function buildMapHtml(spots: TourismSpot[], isAr: boolean, apiBase: string, userLat: number, userLng: number): string {
...
#filter-bar{position:absolute;top:115px;${side}:10px;z-index:1000;display:flex;flex-direction:column;gap:5px;}
.fbtn{padding:4px 13px;border-radius:13px;border:1px solid rgba(255,255,255,0.22);background:rgba(10,22,40,0.84);color:#cbd5e1;font-size:11px;font-family:-apple-system,'Segoe UI',Tahoma,sans-serif;font-weight:600;cursor:pointer;white-space:nowrap;min-width:74px;text-align:center;-webkit-tap-highlight-color:transparent;user-select:none;transition:background .12s,border-color .12s;}
...
#legend{position:absolute;bottom:52px;${side}:10px;z-index:1000;background:rgba(10,22,40,0.88);border:1px solid rgba(255,255,255,0.14);border-radius:10px;padding:7px 11px;font-family:-apple-system,'Segoe UI',Tahoma,sans-serif;}
.l-row{display:flex;align-items:center;gap:6px;margin-bottom:4px;}.l-row:last-child{margin-bottom:0;}
.ldot{width:10px;height:10px;border-radius:50%;flex-shrink:0;}
.l-lbl{font-size:10.5px;color:#e2e8f0;font-weight:600;}
#status-bar{position:absolute;top:72px;left:0;right:0;z-index:1001;display:none;align-items:center;justify-content:center;pointer-events:none;}
...
.lf-popup .leaflet-popup-content-wrapper{background:#0f2040;border:1.5px solid #C9A84C;border-radius:14px;box-shadow:0 8px 26px rgba(0,0,0,.65);color:#f1f5f9;padding:0;font-family:-apple-system,'Segoe UI',Tahoma,sans-serif;direction:${dir};}
...
<div id="filter-bar">
  ...
  <button class="fbtn" onclick="filterApt(this)">${isAr ? "🏠 شقق" : "🏠 Apts"}</button>
</div>
<div id="legend">
...
(function(){
  ...
  L.circleMarker([USER_LAT,USER_LNG],{radius:10,fillColor:'#6366F1',color:'#fff',weight:2.5,opacity:1,fillOpacity:.95})
  ...
  function makeTourismPopup(s,pct){var info=busyInfo(pct);var clr=CAT_CLR[s.category]||'#D4A843';var catL=IS_AR?(CAT_AR[s.category]||s.category):(CAT_EN[s.category]||s.category);var barH=pct!==null?'<div class="pop-bar-track"><div class="pop-bar-fill" style="width:'+pct+'%;background:'+info.clr+'"></div></div>':'';var stH=s.rating?'<div class="pop-stars-row"><span class="pop-stars">'+stars(s.rating)+'</span><span class="pop-rating-num">'+s.rating.toFixed(1)+'</span></div>':'';return '<span class="pop-type" style="color:'+clr+'">'+esc(catL)+'</span><div class="pop-name">'+esc(IS_AR?s.nameAr:s.nameEn)+'</div><div class="pop-city">📍 '+esc(IS_AR?s.cityAr:s.cityEn)+'</div>'+stH+'<div class="pop-desc">'+esc(IS_AR?s.descAr:s.descEn)+'</div><div class="pop-busy"><div class="pop-busy-row"><span class="pop-busy-title">'+(IS_AR?'الزحمة':'Busyness')+'</span><span class="pop-busy-val" style="color:'+info.clr+'">'+info.dot+' '+info.lbl+'</span></div>'+barH+'</div><a class="pop-btn" href="'+s.mapsUrl+'" target="_blank">🗺️ '+(IS_AR?'افتح الخريطة':'Open Maps')+'</a>';}
  function renderTourism(){tourismLayer.clearLayers();SPOTS.forEach(function(s){var pct=calcBusy(s.id,s.category);L.marker([s.lat,s.lng],{icon:makeEmojiIcon(s,pct)}).bindPopup(makeTourismPopup(s,pct),{className:'lf-popup',maxWidth:275,closeButton:true}).addTo(tourismLayer);});}
  renderTourism();setInterval(renderTourism,15*60*1000);

  var POI_CFG={attraction:{clr:'#16A34A',emoji:'🎯',arName:'سياحي',enName:'Tourism'},hotel:{clr:'#3B82F6',emoji:'🏨',arName:'فندق',enName:'Hotel'},restaurant:{clr:'#EF4444',emoji:'🍽️',arName:'مطعم',enName:'Restaurant'},cafe:{clr:'#92400E',emoji:'☕',arName:'كافيه',enName:'Café'},historic:{clr:'#D97706',emoji:'🏛️',arName:'تاريخي',enName:'Historic'}};
  var APTS={};

  /* Open ride app — web uses window.open */
  ...
  function makePOIPopup(p,cfg){var name=IS_AR?(p.nameAr||p.nameEn||''):(p.nameEn||p.nameAr||'');if(!name)name=IS_AR?'اسم غير متاح':'N/A';var tags=p.tags||{};var phone=tags.phone||tags['contact:phone']||tags['contact:mobile']||'';var addr=tags['addr:full']||'';if(!addr){var ap=[];if(tags['addr:city'])ap.push(tags['addr:city']);if(tags['addr:street'])ap.push(tags['addr:street']);addr=ap.join(' — ');}var h='<div class="pop-type" style="color:'+cfg.clr+'">'+cfg.emoji+' '+(IS_AR?cfg.arName:cfg.enName)+'</div>';h+='<div class="pop-name">'+esc(name)+'</div>';if(addr)h+='<div class="pop-addr">📍 '+esc(addr)+'</div>';h+=phone?'<a class="pop-call" href="tel:'+phone.replace(/\\s/g,'')+'">📞 '+(IS_AR?'اتصال':'Call')+'</a>':'<div class="pop-no-phone">'+(IS_AR?'لا يوجد هاتف':'No phone')+'</div>';return h;}
  ...
  function loadPoi(type){
    ...
    fetch(url)
      ...
      .then(function(data){
        poiLayer.clearLayers();Object.keys(APTS).forEach(function(k){delete APTS[k];});
        ...
        places.forEach(function(p){
          ...
          L.circleMarker([p.lat,p.lng],{radius:7,fillColor:clr,color:'#fff',weight:1.5,opacity:1,fillOpacity:.88})
  ...
  window.filterPoi=function(btn,type){clearActive();btn.classList.add('active');map.addLayer(tourismLayer);loadPoi(type);};
...
export default function TourismMapView({ spots = [], isAr = false, apiBase = "", userLat = DEFAULT_LAT, userLng = DEFAULT_LNG }: Props) {
