 ...
 * Immersive 4D villa: day/night + rain + 5 Saudi interior styles.
 */
  ...
  return (
    <View style={StyleSheet.absoluteFill}>
      <WebView
        ...
        onLoad={() => { setTimeout(() => onReady?.(), 1800); }}
      />
    </View>
  );
}

const s = StyleSheet.create({ wv: { flex: 1, backgroundColor: "#87ceeb" } });
...
.btn{background:rgba(15,32,64,0.88);border:1.5px solid rgba(201,168,76,0.55);color:#c9a84c;
  ...
  box-shadow:0 3px 12px rgba(0,0,0,0.3);transition:background 0.15s}
.btn:active{background:rgba(201,168,76,0.18)}
.btn.active{background:rgba(201,168,76,0.25);border-color:#c9a84c;color:#f5d98a}
.sBtn{background:rgba(15,32,64,0.78);border:1.5px solid rgba(255,255,255,0.18);color:rgba(255,255,255,0.8);
...
#weatherBar{position:absolute;top:14px;left:50%;transform:translateX(-50%);
  background:rgba(15,32,64,0.85);border:1px solid rgba(201,168,76,0.35);border-radius:20px;
  padding:5px 16px;color:#c9a84c;font-size:12px;font-weight:700;letter-spacing:0.4px;
  pointer-events:none;white-space:nowrap}
#styleName{position:absolute;top:50px;left:50%;transform:translateX(-50%);
  background:rgba(15,32,64,0.75);border:1px solid rgba(201,168,76,0.3);border-radius:14px;
  padding:4px 14px;color:rgba(201,168,76,0.9);font-size:11px;font-weight:700;
...
sunLight.shadow.camera.top=25;sunLight.shadow.camera.bottom=-25;scene.add(sunLight);
...
var ground=new THREE.Mesh(new THREE.PlaneGeometry(80,80),groundM);
ground.rotation.x=-Math.PI/2;ground.position.y=-1.02;ground.receiveShadow=true;scene.add(ground);
var pathMesh=new THREE.Mesh(new THREE.PlaneGeometry(2,8),new THREE.MeshLambertMaterial({color:0xc8b48a}));
pathMesh.rotation.x=-Math.PI/2;pathMesh.position.set(0,-1.01,7);scene.add(pathMesh);

// ── Villa ──────────────────────────────────────────────────────────────────
var villaG=new THREE.Group();
var bodyM=new THREE.MeshLambertMaterial({color:0xf8fafc});
var body=new THREE.Mesh(new THREE.BoxGeometry(8,4,6),bodyM);
body.position.y=1;body.castShadow=true;body.receiveShadow=true;villaG.add(body);
var floor2M=new THREE.MeshLambertMaterial({color:0xede8de});
var floor2=new THREE.Mesh(new THREE.BoxGeometry(5,2.5,5),floor2M);
floor2.position.set(-0.5,4.25,0.5);floor2.castShadow=true;villaG.add(floor2);
var roofM=new THREE.MeshLambertMaterial({color:0x8b3a1a});
var roofMesh=new THREE.Mesh(new THREE.ConeGeometry(5.2,2.5,4),roofM);
roofMesh.position.set(0,4.5,0);roofMesh.rotation.y=Math.PI/4;roofMesh.castShadow=true;villaG.add(roofMesh);
var garage=new THREE.Mesh(new THREE.BoxGeometry(3.5,2.5,3),new THREE.MeshLambertMaterial({color:0xe8e0d0}));
garage.position.set(4.5,0.25,-0.5);garage.castShadow=true;villaG.add(garage);
var gRoof=new THREE.Mesh(new THREE.BoxGeometry(3.8,0.2,3.3),new THREE.MeshLambertMaterial({color:0x7a3318}));
gRoof.position.set(4.5,1.6,-0.5);villaG.add(gRoof);
...
// ── Interior furniture (porch / visible front area) ─────────────────────
var sofaMat=new THREE.MeshLambertMaterial({color:0xe5e7eb});
var sofaMesh=new THREE.Mesh(new THREE.BoxGeometry(2.5,0.6,0.9),sofaMat);
sofaMesh.position.set(0,-0.7,4.6);sofaMesh.castShadow=true;scene.add(sofaMesh);
// Sofa back
var sofaBack=new THREE.Mesh(new THREE.BoxGeometry(2.5,0.65,0.18),sofaMat);
sofaBack.position.set(0,-0.38,4.16);scene.add(sofaBack);
// Sofa arms
var sofaArm1=new THREE.Mesh(new THREE.BoxGeometry(0.18,0.5,0.9),sofaMat);
sofaArm1.position.set(1.15,-0.55,4.6);scene.add(sofaArm1);
var sofaArm2=new THREE.Mesh(new THREE.BoxGeometry(0.18,0.5,0.9),sofaMat);
sofaArm2.position.set(-1.15,-0.55,4.6);scene.add(sofaArm2);
// Coffee table
var woodMat=new THREE.MeshLambertMaterial({color:0x92400e});
var tableMesh=new THREE.Mesh(new THREE.BoxGeometry(1.2,0.1,0.7),woodMat);
tableMesh.position.set(0,-0.95,5.5);tableMesh.castShadow=true;scene.add(tableMesh);
// Table legs
[[-0.5,0,-0.28],[0.5,0,-0.28],[-0.5,0,0.28],[0.5,0,0.28]].forEach(function(p){
  var leg=new THREE.Mesh(new THREE.CylinderGeometry(0.04,0.04,0.4,6),woodMat);
  leg.position.set(tableMesh.position.x+p[0],tableMesh.position.y-0.25,tableMesh.position.z+p[2]);
  scene.add(leg);
});
// Rug / floor mat under sofa area (shaabi style shows this prominently)
var rugMat=new THREE.MeshLambertMaterial({color:0x991b1b});
var rugMesh=new THREE.Mesh(new THREE.PlaneGeometry(3.2,2.0),rugMat);
rugMesh.rotation.x=-Math.PI/2;rugMesh.position.set(0,-1.01,5.0);scene.add(rugMesh);
// Decorative vase
var vaseMat=new THREE.MeshLambertMaterial({color:0xc9a84c});
var vaseMesh=new THREE.Mesh(new THREE.CylinderGeometry(0.12,0.08,0.4,10),vaseMat);
vaseMesh.position.set(-0.5,-0.8,5.5);scene.add(vaseMesh);
...
function addTree(x,z){
  var t=new THREE.Mesh(new THREE.CylinderGeometry(0.15,0.2,1.8,8),new THREE.MeshLambertMaterial({color:0x5c3d1e}));
  t.position.set(x,-0.1,z);t.castShadow=true;scene.add(t);
  var l=new THREE.Mesh(new THREE.SphereGeometry(1.2,10,10),new THREE.MeshLambertMaterial({color:0x2d7a2d}));
  l.position.set(x,1.5,z);l.castShadow=true;scene.add(l);
}
addTree(-8,2);addTree(-8,-3);addTree(-7,6);addTree(10,-5);addTree(-5,-7);addTree(8,-7);

// ── Stars / Moon ───────────────────────────────────────────────────────────
var starPos=new Float32Array(2000*3);
for(var si=0;si<2000;si++){var th=Math.random()*Math.PI*2,ph=Math.random()*Math.PI*0.5,sr=180+Math.random()*40;starPos[si*3]=sr*Math.sin(ph)*Math.cos(th);starPos[si*3+1]=Math.abs(sr*Math.cos(ph))+10;starPos[si*3+2]=sr*Math.sin(ph)*Math.sin(th);}
...
// ── Style definitions ──────────────────────────────────────────────────────
var STYLES={
  modern:  {wall:0xf8fafc,floor2:0xf8fafc,sofa:0xe5e7eb,wood:0x92400e,pool:0x06b6d4,rug:0x991b1b,vase:0xc9a84c,roof:0x8b3a1a,shaabi:false,name:'🏠 مودرن'},
  classic: {wall:0xfefce8,floor2:0xfefce8,sofa:0xb45309,wood:0x78350f,pool:0x06b6d4,rug:0x7c2d12,vase:0xd4af37,roof:0x7a2d0f,shaabi:false,name:'🕌 كلاسيك'},
  neo:     {wall:0xf1f5f9,floor2:0xf1f5f9,sofa:0x1e293b,wood:0xa16207,pool:0x06b6d4,rug:0x1e3a5f,vase:0xa16207,roof:0x374151,shaabi:false,name:'✨ نيو كلاسيك'},
  shaabi:  {wall:0xf5f5f4,floor2:0xf5f5f4,sofa:0x78716c,wood:0x451a03,pool:0x06b6d4,rug:0x7c2d12,vase:0x92400e,roof:0x5a3318,shaabi:true, name:'🛋️ شعبي'},
  farm:    {wall:0xecfccb,floor2:0xd9f99d,sofa:0x4d7c0f,wood:0x365314,pool:0x22c55e,rug:0x365314,vase:0x16a34a,roof:0x365314,shaabi:false,name:'🌿 مزرعة'}
};
var currentStyle='modern';

window.setStyle=function(name){
  currentStyle=name;var st=STYLES[name];
  // Update button highlights
  ['modern','classic','neo','shaabi','farm'].forEach(function(k){
    var b=document.getElementById('style-'+k);if(b)b.classList.toggle('active',k===name);
  });
  // Update style label
  document.getElementById('styleName').textContent=st.name;
  // Update materials
  bodyM.color.setHex(st.wall);
  floor2M.color.setHex(st.floor2);
  sofaMat.color.setHex(st.sofa);
  sofaBack.material.color.setHex(st.sofa);
  sofaArm1.material.color.setHex(st.sofa);
  sofaArm2.material.color.setHex(st.sofa);
  woodMat.color.setHex(st.wood);
  waterM.color.setHex(st.pool);
  rugMat.color.setHex(st.rug);
  vaseMat.color.setHex(st.vase);
  roofM.color.setHex(st.roof);
  // Shaabi = floor seating (sofa sinks to ground, sofa back disappears)
  if(st.shaabi){
    sofaMesh.position.y=-0.88;sofaMesh.scale.y=0.45;
    sofaBack.visible=false;sofaArm1.visible=false;sofaArm2.visible=false;
  }else{
    sofaMesh.position.y=-0.7;sofaMesh.scale.y=1;
    sofaBack.visible=true;sofaArm1.visible=true;sofaArm2.visible=true;
  }
  // Farm = green ground
  groundM.color.setHex(st.farm?0x4d7c0f:isNight?0x1a3520:0x4a8c52);
};
...
function startRainAudio(){
  ...
  var f=audioCtx.createBiquadFilter();f.type='lowpass';f.frequency.value=1100;
...
cnv.addEventListener('touchmove',function(e){if(e.touches.length===1&&orb.dragging){orb.theta-=(e.touches[0].clientX-orb.prev.x)/window.innerWidth*Math.PI*2.2;orb.phi=Math.max(0.15,Math.min(Math.PI/2.05,orb.phi+(e.touches[0].clientY-orb.prev.y)/window.innerHeight*Math.PI));orb.prev={x:e.touches[0].clientX,y:e.touches[0].clientY};orbitToCart();}if(e.touches.length===2){var d=Math.hypot(e.touches[0].clientX-e.touches[1].clientX,e.touches[0].clientY-e.touches[1].clientY);if(tPrev!==null){orb.r=Math.max(8,Math.min(50,orb.r-(d-tPrev)*0.05));orbitToCart();}tPrev=d;}},{passive:true});
...
function animate(){
  requestAnimationFrame(animate);var dt=clock.getDelta();var t=clock.getElapsedTime();
  ...
  ambLight.intensity=lerp(0.65,0.1,n);sunLight.intensity=lerp(1.4,0.0,n);
  moonLight.intensity=lerp(0.0,0.6,n);poolGlow.intensity=lerp(0.0,1.8,n);
  starMat.opacity=lerp(0,0.95,n);moonM2.opacity=lerp(0,0.92,n);
  var st=STYLES[currentStyle];
  groundM.color.setHex(n>0.5?0x1a3520:(st===STYLES.farm?0x4d7c0f:0x4a8c52));
  ...
  if(rainProgress>0.05){
    var rp=rainGeo.attributes.position.array;var spd=18*rainProgress;
    for(var ri=0;ri<3000;ri++){rp[ri*3+1]-=spd*dt;if(rp[ri*3+1]<-2)rp[ri*3+1]=30;}
