 ...
 *
 * Builds the full Leaflet map HTML string SYNCHRONOUSLY inside React Native,
 * exactly like HeatmapMapView does — no fetch, no server dependency.
 *
 * This is the ONLY approach proven to work on Android WebView in this app:
 *  • source={{ html: string }}  (no baseUrl)  ← HeatmapMapView pattern
 *  • Leaflet from unpkg CDN
 *  • Carto rastertiles/voyager tiles
 *  • POI fetch calls use absolute apiBase URL embedded as JS var
 */
...
/* ── HTML builder (runs in RN JS thread, no network needed) ─────────────── */
function buildHtml(opts: {
  isAr: boolean; lat: number; lng: number; hasTabs: boolean; apiBase: string;
}): string {
  const { isAr, lat, lng, hasTabs, apiBase } = opts;
  const dir        = isAr ? "rtl" : "ltr";
  const spotsJson  = JSON.stringify(SPOTS);

  /* Legend labels bottom offset: tab bar visible → 110px, else 60px */
...
(function(){
  ...
  /* ── Map init ── */
  var map=L.map('map',{center:[USER_LAT,USER_LNG],zoom:6,
  ...
  /* Force size recalc after WebView finishes painting */
  ...
  /* ── POI layer (optional — from API server) ── */
  var POI_CFG={
...
export default function TourismMapView({
  ...
  const key = `${userLat.toFixed(4)}_${userLng.toFixed(4)}_${isAr}_${hasTabs}`;

  if (!htmlRef.current || prevKey.current !== key) {
    prevKey.current = key;
    htmlRef.current = buildHtml({ isAr, lat: userLat, lng: userLng, hasTabs, apiBase });
  }
