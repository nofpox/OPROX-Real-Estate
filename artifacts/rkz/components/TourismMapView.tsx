...
<style>
  *{box-sizing:border-box;margin:0;padding:0;}
  ...
  .ldot{width:12px;height:12px;border-radius:50%;border:2px solid rgba(255,255,255,0.45);flex-shrink:0;}
  .l-lbl{font-size:12px;color:#e2e8f0;font-weight:600;}
  ...
  #loading .spin{font-size:26px;display:block;margin-bottom:10px;animation:spin 1.2s linear infinite;}
  @keyframes spin{to{transform:rotate(360deg);}}
  #loading .ltxt{font-size:14px;color:#C9A84C;font-weight:700;}
  #loading .lsub{font-size:11px;color:rgba(255,255,255,0.40);margin-top:5px;}
  ...
  .pop-call:active{background:#15803d;}
...
<div id="loading">
  ...
  <div class="lsub">Overpass API</div>
</div>
...
(function(){
  ...
  var layers={hotel:L.layerGroup().addTo(map),rest:L.layerGroup().addTo(map),cafe:L.layerGroup().addTo(map)};
  ...
  function getAddr(tags){
    ...
    if(p.length===0&&tags.city) p.push(tags.city);
  ...
  function makePopup(type,tags){
    ...
    if(rating){
      h+='<div class="pop-rating">⭐ '+esc(String(rating))+'</div>';
    } else {
    ...
    if(phone){
      ...
      h+='<a class="pop-call" href="tel:'+cp+'">📞 '+(IS_AR?'اتصال':'Call')+'</a>';
    } else {
  ...
  function addMarker(type,el){
    ...
    var tags=el.tags||{};
    if(!getName(tags)) return;
    L.circleMarker([el.lat,el.lon],{
      radius:8,fillColor:COLORS[type],
      color:'#fff',weight:1.5,opacity:1,fillOpacity:0.90
    })
    .bindPopup(makePopup(type,tags),{className:'lf-popup',maxWidth:270,closeButton:true})
    .addTo(layers[type]);
  }
  ...
  function fetchType(type,query){
    fetch(OVP,{method:'POST',
      headers:{'Content-Type':'application/x-www-form-urlencoded'},
      body:'data='+encodeURIComponent(query)
    })
    .then(function(r){return r.json();})
    .then(function(data){(data.elements||[]).forEach(function(el){addMarker(type,el);});done();})
    .catch(function(){done();});
  }
