...
export default function Landing() {
  ...
  const handleAccess = () => {
    ...
    if (!trimmed || isNaN(Number(trimmed)) || Number(trimmed) <= 0) {
      setError(t("request.invalidUnit"));
      return;
    }
    setError("");
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-amber-50/30 to-white flex flex-col">
      ...
      <header className="px-6 pt-10 pb-6 text-center relative">
        ...
        {/* Arabic brand name — large serif */}
        <h1 className="text-2xl font-serif font-bold text-slate-900 leading-snug" dir="rtl">
          ركز للحلول الذكية
        </h1>
        <p className="text-xs font-semibold tracking-widest text-amber-600 uppercase mt-0.5 mb-1">
          Rakez Smart Solutions
        </p>
        <p className="mt-1 text-slate-500 text-sm">{t("request.serviceRequest")}</p>
      </header>
      ...
      <main className="flex-1 px-6 flex flex-col items-center justify-center -mt-8">
        <Card className="w-full max-w-sm shadow-xl border-0 rounded-3xl overflow-hidden">
          <div className="bg-gradient-to-r from-amber-500 to-amber-400 px-6 py-5">
            <h2 className="text-xl font-bold text-white">{t("landing.accessUnit")}</h2>
            <p className="text-amber-100 text-sm mt-0.5">{t("landing.enterUnit")}</p>
          </div>
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-8">
          {[
            { icon: Wrench, label: t("landing.submitRequests"), color: "bg-blue-50 text-blue-700" },
            { icon: Star,   label: t("landing.rateStay"),       color: "bg-purple-50 text-purple-700" },
            { icon: Wifi,   label: t("landing.unitDetails"),    color: "bg-green-50 text-green-700" },
