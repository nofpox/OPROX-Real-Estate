...
import { Plus, Users, MoreVertical, Phone, Mail, Building2, UserCheck, UserX } from "lucide-react";
...
function getRoleColor(role: string): string {
  ...
  if (role.includes("Desk") || role.includes("Concierge") || role.includes("Reception")) return "bg-amber-100 text-amber-800 dark:bg-amber-900/30 dark:text-amber-400";
...
function getAvatarColor(name: string): string {
  const colors = [
    ...
    "bg-red-500", "bg-indigo-500", "bg-teal-500", "bg-rose-500",
  ];
...
export default function Staff() {
  ...
  const uniqueRoleCategories = ["Management", "Housekeeping", "Maintenance", "Security", "Reception"];
  ...
  const handleToggleStatus = (s: any) => {
    ...
    updateStaff.mutate({ id: s.id, data: { status: newStatus } }, {
      onSuccess: () => {
        toast({ title: `Staff member marked as ${newStatus}` });
        queryClient.invalidateQueries({ queryKey: getListStaffQueryKey(params) });
      },
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage your team across all properties.</p>
        </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card className="shadow-sm">
          <CardContent className="p-6">
            ...
            <h2 className="text-3xl font-bold">{staff?.length || 0}</h2>
          </CardContent>
        </Card>
        <Card className="shadow-sm">
          <CardContent className="p-6">
            ...
            <h2 className="text-3xl font-bold text-green-600 dark:text-green-500">{activeCount}</h2>
          </CardContent>
        </Card>
        <Card className="shadow-sm">
          <CardContent className="p-6">
            ...
            <h2 className="text-3xl font-bold text-muted-foreground">{inactiveCount}</h2>
          </CardContent>
        </Card>
      </div>
          ...
          {filteredStaff.map((s) => (
            <Card key={s.id} className="shadow-sm hover:shadow-md transition-shadow border-border/50">
              <CardContent className="p-5">
                <div className="flex items-start gap-4">
                  <Avatar className={`h-12 w-12 border-2 border-background shadow-sm ${getAvatarColor(s.name)}`}>
                    <AvatarFallback className={`text-white font-semibold text-sm ${getAvatarColor(s.name)}`}>
                      {getInitials(s.name)}
                    </AvatarFallback>
                  </Avatar>
                  <div className="flex-1 min-w-0">
                    ...
                    <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium mt-1 ${getRoleColor(s.role)}`}>
                      {s.role}
                    </span>
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem className="col-span-2">
                      ...
                      <FormControl>
                        <Input placeholder="e.g. Sarah Collins" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input type="email" placeholder="email@property.com" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
