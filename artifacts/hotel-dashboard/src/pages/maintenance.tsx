...
const workOrderSchema = z.object({
  ...
  status: z.enum(["open", "in-progress", "on-hold", "completed"]).default("open"),
...
const StatusBadge = ({ status }: { status: string }) => {
  switch (status) {
    case 'open': return <Badge className="bg-blue-100 text-blue-800 hover:bg-blue-100 border-0 dark:bg-blue-900/30 dark:text-blue-400">Open</Badge>;
...
export default function Maintenance() {
  ...
  const onSubmit = (data: z.infer<typeof workOrderSchema>) => {
    ...
    createWorkOrder.mutate({ data: payload }, {
      onSuccess: () => {
  ...
  const stats = {
    open: allWorkOrders?.filter(w => w.status === 'open').length || 0,
                    ...
                    <TableRow key={wo.id}>
                      ...
                      <TableCell>
                        <div className="flex flex-col">
                          ...
                          <span className="text-xs text-muted-foreground">
                            {wo.propertyName}{wo.unitName ? ` • Unit: ${wo.unitName}` : ' • Property-wide'}
                          </span>
                        </div>
                      </TableCell>
                      <TableCell className="text-sm">{wo.assignedTo || <span className="text-muted-foreground italic">Unassigned</span>}</TableCell>
                      ...
                      <TableCell className="text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Select 
                            value={wo.status} 
                            onValueChange={(val) => handleStatusChange(wo.id, val)}
                          >
