...
import {
  ArrowLeft, Building2, AlertCircle,
...
export default function WorkerUnitDetail({ id }: { id: number }) {
  ...
  const updateStatus = useMutation({
    ...
    onSuccess: () => {
      toast({ title: t("worker.unitDetail.unitStatus") });
      queryClient.invalidateQueries({ queryKey: ["room", id] });
      queryClient.invalidateQueries({ queryKey: ["rooms"] });
    },
    onError: () => toast({ title: t("worker.unitDetail.unitStatus"), variant: "destructive" }),
  });
  ...
  const updateFinancial = useMutation({
    ...
    onSuccess: () => {
      toast({ title: t("worker.unitDetail.financialData") });
      queryClient.invalidateQueries({ queryKey: ["financial", id] });
      setEditFinancial(false);
    },
    onError: () => toast({ title: t("worker.unitDetail.saveChanges"), variant: "destructive" }),
  });
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-10 bg-card border-b border-border/50 px-6 py-4 flex items-center gap-4">
        <button onClick={() => navigate("/worker")} className="text-muted-foreground hover:text-foreground">
          <ArrowLeft size={20} />
        </button>
        <div className="flex items-center gap-2">
          <Building2 size={17} className="text-amber-500" />
          <span className="font-serif font-bold text-foreground">
            {roomLoading ? t("worker.unitDetail.loading") : room?.name}
          </span>
        </div>
        {room && (
      ...
      <div className="px-6 py-6 max-w-2xl mx-auto space-y-5">
        ...
        <Card className="border-border/50">
          <CardHeader className="pb-3">
            <CardTitle className="text-sm font-semibold text-foreground flex items-center gap-2">
              <Building2 size={15} className="text-amber-500" /> {t("worker.unitDetail.unitStatus")}
            </CardTitle>
          </CardHeader>
          <CardContent>
              ...
              <div className="flex gap-2">
                <Select value={room?.status} onValueChange={v => {
                  if (!isOnline) {
                    ...
                    toast({ title: t("worker.unitDetail.offline") });
                    return;
                  }
                  updateStatus.mutate(v);
                }}>
                  <SelectTrigger className="flex-1">
                    <SelectValue placeholder={t("worker.unitDetail.setStatus")} />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <Button variant="outline" size="icon" onClick={() => setQrOpen(true)} title={t("worker.unitDetail.qrTitle")}>
                  <QrCode size={16} />
                </Button>
              </div>
            )}
            {room && (
              <div className="mt-3 grid grid-cols-2 gap-2 text-xs text-muted-foreground">
                <span>{t("worker.unitDetail.type")}: <strong className="text-foreground">{room.type}</strong></span>
                <span>{t("worker.unitDetail.capacity")}: <strong className="text-foreground">{room.capacity} {t("worker.unitDetail.guests")}</strong></span>
                <span>{t("worker.unitDetail.rate")}: <strong className="text-foreground">${room.pricePerNight}{t("worker.unitDetail.perNight")}</strong></span>
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="border-border/50">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-semibold text-foreground flex items-center gap-2">
                <DollarSign size={15} className="text-amber-500" /> {t("worker.unitDetail.financialData")}
              </CardTitle>
              <Button size="sm" variant="ghost" onClick={() => setEditFinancial(!editFinancial)} className="h-7 text-xs text-muted-foreground">
                {editFinancial ? t("worker.unitDetail.cancel") : t("worker.unitDetail.edit")}
              </Button>
            </div>
          </CardHeader>
          <CardContent>
            {finLoading ? (
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <Label className="text-xs">{t("worker.unitDetail.status")}</Label>
                    <Select value={financialForm.status || ""} onValueChange={v => setFinancialForm(f => ({ ...f, status: v }))}>
                      <SelectTrigger className="mt-1 h-9 text-xs"><SelectValue placeholder={t("worker.unitDetail.status")} /></SelectTrigger>
                      <SelectContent>
                  ...
                  <div>
                    <Label className="text-xs">{t("worker.unitDetail.amountDue")}</Label>
                  ...
                  <div>
                    <Label className="text-xs">{t("worker.unitDetail.dueDate")}</Label>
                  ...
                  <div>
                    <Label className="text-xs">{t("worker.unitDetail.checkIn")}</Label>
                  ...
                  <div>
                    <Label className="text-xs">{t("worker.unitDetail.checkOut")}</Label>
                ...
                <Button onClick={() => updateFinancial.mutate()} disabled={updateFinancial.isPending} className="w-full h-9 bg-amber-500 hover:bg-amber-600 text-black font-semibold">
                  {updateFinancial.isPending ? <Loader2 className="animate-spin" size={15} /> : t("worker.unitDetail.saveChanges")}
                </Button>
              </div>
            ) : financial ? (
              <div className="grid grid-cols-2 gap-y-3 text-xs">
                {([
                  [t("worker.unitDetail.status"), <span className="capitalize">{financial.status}</span>],
                  [t("worker.unitDetail.amountDue"), financial.amountDue != null ? `$${financial.amountDue}` : "—"],
                  [t("worker.unitDetail.dueDate"), financial.dueDate || "—"],
                  [t("worker.unitDetail.checkIn"), financial.checkIn || "—"],
                  [t("worker.unitDetail.checkOut"), financial.checkOut || "—"],
              ...
              <div className="text-center py-4">
                <p className="text-sm text-muted-foreground">{t("worker.unitDetail.noFinancial")}</p>
                <Button size="sm" variant="outline" className="mt-2 text-xs" onClick={() => setEditFinancial(true)}>
                  {t("worker.unitDetail.addFinancial")}
                </Button>
              </div>
            )}
          </CardContent>
        ...
        <Card className="border-border/50">
          <CardHeader className="pb-3">
            <div className="flex items-center justify-between">
              <CardTitle className="text-sm font-semibold text-foreground flex items-center gap-2">
                <AlertCircle size={15} className="text-amber-500" /> {t("worker.unitDetail.serviceRequests")}
                {requests && requests.filter(r => r.status === "new").length > 0 && (
                  <Badge className="bg-red-500/20 text-red-400 border-red-500/30 text-[10px] ml-1">
                    {requests.filter(r => r.status === "new").length} {t("worker.unitDetail.new")}
                  </Badge>
                )}
              </CardTitle>
          ...
          <CardContent>
            {!requests || requests.length === 0 ? (
              <p className="text-center text-sm text-muted-foreground py-4">{t("worker.unitDetail.noRequests")}</p>
            ) : (
                  ...
                  <div key={req.id} className="flex items-start gap-3 p-3 rounded-lg bg-background border border-border/30">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${REQ_STATUS_BADGE[req.status] || "bg-muted text-muted-foreground"}`}>
                          {req.status === "in_progress" ? t("status.in-progress") : req.status.charAt(0).toUpperCase() + req.status.slice(1)}
                        </span>
                      </div>
                    ...
                    {req.status !== "resolved" && (
                      <Button size="sm" variant="ghost" onClick={() => {
                        ...
                        <CheckCircle2 size={13} className="mr-1" /> {t("worker.unitDetail.resolve")}
                      </Button>
                    )}
                  </div>
                ))}
