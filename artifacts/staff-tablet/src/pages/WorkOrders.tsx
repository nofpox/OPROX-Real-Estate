...
const STATUS_AR: Record<string, string> = {
  pending: "معلّق", "in-progress": "جارٍ", completed: "مكتمل", cancelled: "ملغى",
};
...
export default function WorkOrders() {
  ...
  const TABS = [
    { key: "pending"     as const, labelAr: "معلّقة",    labelEn: "Pending",     count: pendingCount    },
    { key: "in-progress" as const, labelAr: "جارية",     labelEn: "In Progress", count: inProgressCount },
    { key: "completed"   as const, labelAr: "مكتملة",    labelEn: "Done",        count: completedCount  },
    { key: "all"         as const, labelAr: "الكل",      labelEn: "All",         count: workOrders?.length ?? 0 },
  ];
  ...
  async function advanceStatus(wo: WorkOrder) {
    ...
    try {
      ...
      toast({ title: next === "in-progress" ? "بدأت العمل · Started" : "تم الإنجاز · Completed" });
      ...
      toast({ title: "فشل التحديث · Update failed", variant: "destructive" });
    } finally {
      setUpdatingId(null);
    }
  }
      ...
      <div className="min-h-screen bg-background pb-24">
        ...
        <header className="sticky top-0 z-10 bg-card border-b border-border/50 px-5 py-4">
          <div className="flex items-center justify-between mb-3">
            <div>
              <h1 className="font-serif font-bold text-foreground text-xl leading-tight">أوامر العمل</h1>
          ...
          <div className="flex gap-2 overflow-x-auto pb-0.5 no-scrollbar">
            {TABS.map(t => (
              <button
                ...
                className={`flex-shrink-0 flex flex-col items-center px-3 py-1.5 rounded-xl text-xs font-semibold transition-all border ${
                  tab === t.key
        ...
        <div className="px-4 py-4 space-y-3">
          ...
          {isError && (
            <div className="text-center py-16">
              <p className="text-muted-foreground text-sm">فشل التحميل · Failed to load</p>
          ...
          {!isLoading && !isError && filtered.length === 0 && (
            <div className="text-center py-16">
              ...
              <p className="text-muted-foreground text-sm font-medium">لا توجد أوامر عمل</p>
              <p className="text-muted-foreground/50 text-xs mt-1">No work orders assigned</p>
            </div>
          )}
            ...
            <div key={wo.id} className="bg-card border border-border/50 rounded-2xl p-4 space-y-3">
              ...
              <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-muted-foreground">
                ...
                {wo.dueDate && (
                  <span className="flex items-center gap-1">
                    <Clock size={10} />
                    {new Date(wo.dueDate + "T00:00:00").toLocaleDateString("ar-SA")}
                  </span>
                )}
              ...
              {wo.status === "in-progress" && (
                ...
                >
                  ...
                  إنهاء العمل · Complete
                </Button>
              )}
              {wo.status === "completed" && (
                <div className="flex items-center justify-center gap-2 py-1">
                  <span className="text-sm text-emerald-400 font-semibold">✓ مكتمل · Completed</span>
                </div>
              )}
            </div>
          ))}
