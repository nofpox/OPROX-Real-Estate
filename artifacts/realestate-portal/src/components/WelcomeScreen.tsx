...
const TARGET     = 82;
const FILL_FRAC  = TARGET / 100;
const OFFSET_END = CIRCUM * (1 - FILL_FRAC);
...
export function WelcomeScreen({ onComplete }: Props) {
  ...
  useEffect(() => {
    const timers = [
      ...
      setTimeout(() => setPhase(5), T_SKIP),
      setTimeout(() => setPhase(6), T_REASON),
      setTimeout(() => setPhase(7), T_CTA),
  ...
  return (
    ...
    >
      ...
      <div style={{
        opacity: phase >= 1 ? 1 : 0,
        transform: phase >= 1 ? "scale(1) translateY(0)" : "scale(0.88) translateY(6px)",
