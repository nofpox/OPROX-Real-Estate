 ...
 *
 * Serves the full Leaflet map page as HTML.
 * Serving from a real HTTPS URL (not inline srcdoc) so CDN scripts load
 * reliably on Android WebView without origin restrictions.
 *
 * POI fetch calls inside the HTML use relative paths (/api/poi…)
 * which resolve correctly against this same origin.
 */
...
/* Filter bar — horizontal row at top, RTL-aware */
#filter-bar{
  position:absolute;top:10px;
  ${isAr ? "right:8px;left:8px;" : "left:8px;right:8px;"}
  z-index:1000;
  display:flex;
  flex-direction:row;
...
.fbtn{
  ...
  font-size:13px;font-family:-apple-system,'Segoe UI',Tahoma,sans-serif;font-weight:700;
...
/* Legend */
#legend{
  position:absolute;bottom:50px;${isAr ? "left:10px" : "right:10px"};
  z-index:1000;
  background:rgba(8,18,36,0.9);border:1px solid rgba(255,255,255,0.12);
...
/* Status pill */
#status-bar{
  position:absolute;top:56px;left:0;right:0;z-index:1001;
...
.spot-pin{
  ...
  box-shadow:0 4px 14px rgba(0,0,0,.7);border:2px solid rgba(255,255,255,.18);background:rgba(10,22,40,.55);
}
...
.star-lbl{
  ...
  font-family:-apple-system,'Segoe UI',sans-serif;box-shadow:0 2px 6px rgba(0,0,0,.5);
}
...
.ride-btn{
  ...
  font-family:-apple-system,'Segoe UI',Tahoma,sans-serif;-webkit-tap-highlight-color:transparent;
}
...
(function(){
  ...
  var map=L.map('map',{center:[USER_LAT,USER_LNG],zoom:12,zoomControl:false,attributionControl:true,tap:true,tapTolerance:15});
  ...
  L.circleMarker([USER_LAT,USER_LNG],{radius:9,fillColor:'#6366F1',color:'#fff',weight:2.5,opacity:1,fillOpacity:.95})
  ...
  function makeEmojiIcon(s,pct){
    ...
    return L.divIcon({html:'<div class="spot-wrap"><div class="spot-pin'+fc+'" style="border-color:'+clr+'">'+s.emoji+'<div class="busy-ring ring-'+info.cls+'"></div></div>'+rl+'</div>',className:'spot-icon',iconSize:[0,0],iconAnchor:[0,0]});
  }
  ...
  function renderTourism(){
    ...
    SPOTS.forEach(function(s){
      ...
      L.marker([s.lat,s.lng],{icon:makeEmojiIcon(s,pct)})
        .bindPopup(makeTourismPopup(s,pct),{className:'lf-popup',maxWidth:270,closeButton:true}).addTo(tourismLayer);
    });
  }
  ...
  window.openRide=function(app,apId){
    ...
    if(window.ReactNativeWebView){window.ReactNativeWebView.postMessage(JSON.stringify({type:'openUrl',url:url}));}
    else{window.open(url,'_system');}
  };

  function makeAptPopup(p){
    var name=p.nameAr||p.nameEn||'—';var tags=p.tags||{};
    var price=tags.price_per_night?'SAR '+Number(tags.price_per_night).toLocaleString('en')+(IS_AR?' / ليلة':' / night'):'';
    var phone=tags.phone||'';var apId='apt_'+p.osmId;
    ...
    h+=phone?'<a class="pop-call" href="tel:'+phone+'">📞 '+(IS_AR?'اتصال':'Call')+'</a>':'<div class="pop-no-phone">'+(IS_AR?'لا يوجد هاتف':'No phone')+'</div>';
  ...
  function makePOIPopup(p,cfg){
    var name=IS_AR?(p.nameAr||p.nameEn||''):(p.nameEn||p.nameAr||'');if(!name)name=IS_AR?'اسم غير متاح':'N/A';
    var tags=p.tags||{};var phone=tags.phone||tags['contact:phone']||tags['contact:mobile']||'';
    var addr=tags['addr:full']||'';if(!addr){var ap=[];if(tags['addr:city'])ap.push(tags['addr:city']);if(tags['addr:street'])ap.push(tags['addr:street']);addr=ap.join(' — ');}
  ...
  function loadPoi(type){
    ...
    fetch(url)
      ...
      .catch(function(){showStatus(IS_AR?'تعذّر التحميل':'Load failed');setTimeout(function(){showStatus(null);},2500);});
  }
