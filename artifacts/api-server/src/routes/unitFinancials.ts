...
router.get("/unit-financials", async (req, res) => {
  ...
  const rows = await db
    ...
    .where(tenantId !== null ? eq(unitFinancialsTable.tenantId, tenantId) : undefined)
...
router.get("/unit-financials/:roomId", async (req, res) => {
  ...
  const tenantId = tid(req);
  const conds = [eq(unitFinancialsTable.roomId, roomId)];
  if (tenantId !== null) conds.push(eq(unitFinancialsTable.tenantId, tenantId));
  const [row] = await db
    .select({ fin: unitFinancialsTable, room: roomsTable, property: propertiesTable })
    .from(unitFinancialsTable)
    .leftJoin(roomsTable, eq(unitFinancialsTable.roomId, roomsTable.id))
...
router.patch("/unit-financials/:roomId", async (req, res) => {
  ...
  if (existing.length > 0) {
    ...
    await db.insert(unitFinancialsTable).values({ tenantId, roomId, ...update } as any);
  }
  const [row] = await db
    .select({ fin: unitFinancialsTable, room: roomsTable, property: propertiesTable })
    .from(unitFinancialsTable)
