...
const PORTAL_SENDER = process.env.SENDER_EMAIL ?? "روزوز للحلول الذكية <onboarding@resend.dev>";
...
router.post("/portal/auth/login-step1", async (req, res) => {
  try {
    ...
    const otp = devOtp ?? String(Math.floor(1000 + Math.random() * 9000));
...
// ── Rozoz preview-guest authentication ────────────────────────────────────
...
router.post("/preview/authenticate", async (req, res) => {
  ...
  try {
    ...
    if (link.portal !== "grand-pms") { res.status(400).json({ error: "Token is not for Rozoz" }); return; }
