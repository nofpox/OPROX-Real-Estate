...
function formatShift(
  ...
  return {
    ...
    staffRole: extras.staffRole ?? null,
...
router.patch("/shifts/:id", async (req, res) => {
  ...
  if (!parsed.success) { res.status(400).json({ error: parsed.error.message }); return; }
  const [shift] = await db.update(shiftsTable).set(parsed.data).where(eq(shiftsTable.id, id)).returning();
